...
class ModelImporterBase(ABC):
    ...
    def loadTextureSlot( self, material, slot, context ):
        ...
        if texturePath in [None, '']:
            ...
            if mip.convert_tex_to_dds and textureTEXPath != None and os.path.exists( textureTEXPath ):
    ...
    def importPrimitives( self, context ):
        ...
        for i in range( len( self.model.primitives ) ):
            ...
            self.importPrimitive( primitive, envelopeIndex, indexStream, vertexStream, context )
    ...
    def importSkeleton( self, context ):
        ...
        for i, joint in enumerate( self.model.joints ):
            ...
            editorBone = self.createBone( joint, jointName, worldMtx, editorParentBone, context ) 
    ...
    def importMaterials( self, context ):
        ...
        for i, materialName in enumerate( self.model.materials ):
            ...
            editorMaterial = self.convertMaterial( material, context, materialName )
            if material != None:    
    ...
    def calcMatrices( self, context ):
        ...
        self.scaleMtx = nclScale( mip.model_scale )
    ...
    def importModel( self, modFilePath, context ):
        ...
        self.importMaterials(context)
        if mip.import_skeleton:
            self.importSkeleton(context)
            self.fixupSkeleton(context)
        if mip.import_groups:
            self.importGroups()
    ...
    @abstractmethod
    def createBone( self, joint, name, tfm, parentBone, context ):
        pass
